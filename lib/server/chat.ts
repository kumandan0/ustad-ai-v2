import { queryTable, readStoredFile } from "@/lib/server/store";
...
async function buildMaterialScopedContext(courseId: number, question: string) {
...
export async function createChatReply(params: {
  ...
  if (!apiKey) {
    if (mode === "materials" && params.courseId) {
      const scoped = await buildMaterialScopedContext(params.courseId, lastUserMessage);
      return [
  ...
  if (mode === "materials") {
    if (!params.courseId) {
      return "Materyal modunu kullanmak icin once bir ders secili olmali.";
    }

    const scoped = await buildMaterialScopedContext(params.courseId, lastUserMessage);

    if (scoped.selectedMaterials.length === 0) {
      return "Bu ders icin henuz yuklenmis PDF, ses veya infografik materyali yok. Once mufredat bolumunden materyal ekleyebilirsin.";
    }
    ...
    return reply || "Bu materyallere dayanarak net bir yanit uretemedim.";
  }
  ...
  return reply || "Bu soru icin yanit uretemedim.";
}
